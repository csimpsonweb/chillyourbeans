...
export default function EquipmentPage() {
  ...
  const fetchEquipmentProducts = useCallback(async () => {
    try {
      setLoading(true);
      // Fetch products from Equipment category (category ID: 4)
      const result = await magentoAPI.getProductsByCategory(4, 50, 1);

      setProducts(result.items);
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch equipment products');
      setProducts([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex space-x-8">
              ...
              <Link href="/equipment" className="text-white font-medium transition-colors">
                Equipment
              </Link>
            </div>
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-black mb-4">Coffee Equipment</h1>
          <p className="text-gray-600">
            Professional coffee equipment and accessories to help you brew the perfect cup at home or in your business.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600">No equipment products found.</p>
          </div>
        ) : (
